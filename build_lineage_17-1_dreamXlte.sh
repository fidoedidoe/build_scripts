...
echoMsg "###       #1. Full LOS $LOS_VERSION Build"
# echoMsg "###       #2. TWRP Recovery Only"
echoMsg "###       #3. LOS Kernel Only"
...
if [[ $SKIP_BUILD_STEPS = "N" ]]; then
  ...
  if [ ! -d "$WORK_DIRECTORY/$REPO_DIRECTORY" ]; then
    ...
    $TIME -f "$TIME_FORMAT" repo init -u https://github.com/Havoc-OS/android_manifest.git -b "$LOCAL_MANIFEST_BRANCH" "$REPO_INIT_FLAG_1" "$REPO_INIT_FLAG_2"
  else
...
if [[ $PROMPT =~ ^[Yy]$ ]]; then
   ...
   case "$BUILD_TYPE" in
    "full")     echoMsg "### Starting $BUILD_TYPE build, running 'brunch $DEVICE_NAME'..."
                ...
                OUT_FILE_NAME="$LOS_VERSION_VANITY-$NOW-$DEVICE_NAME-Unofficial.zip"
    ...
    "kernel")   echoMsg "### Starting $BUILD_TYPE build..."
		...
		export SUBARCH=arm64
                export LOCALVERSION="-$DEVICE_NAME-$GCC_VERSION_VANITY"
	        ...
	        make O="$KERNEL_OUT_DIR" clean
                ...
                OUT_FILE_NAME="$DEVICE_NAME-kernel-$LOS_VERSION_VANITY/-$GCC_VERSION_VANITY.$NOW.zip"
